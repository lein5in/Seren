...
SECRET_KEY = "seren-secret-key-change-in-prod"
ALGORITHM  = "HS256"
...
def update_identity(user_id: int, data: UserIdentity, db: Session = Depends(get_db)):
    user = db.query(UserDB).filter(UserDB.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
def change_password(user_id: int, data: UserPasswordChange, db: Session = Depends(get_db)):
    user = db.query(UserDB).filter(UserDB.id == user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
